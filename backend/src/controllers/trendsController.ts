...
// Mock trend data - 실제로는 데이터베이스나 외부 API에서 가져옴
const mockTrends = [
  {
    id: 1,
...
export const getTrends = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "트렌드를 가져오는 중 오류가 발생했습니다." });
  }
};
...
export const searchTrends = async (req: Request, res: Response) => {
  try {
    ...
    if (!q) {
      return res.status(400).json({ error: "검색어를 제공해주세요." });
    }
